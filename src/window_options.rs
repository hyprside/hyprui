...
impl From<WindowOptions<'_>> for WindowAttributes {
    fn from(options: WindowOptions) -> Self {
        ...
        if options.min_size != (0., 0.) {
            winit_opt = winit_opt
                .with_min_surface_size(LogicalSize::new(options.min_size.0, options.min_size.1));
        }
        ...
        if options.max_size != (0., 0.) {
            winit_opt = winit_opt
                .with_max_surface_size(LogicalSize::new(options.max_size.0, options.max_size.1))
        }
